...
namespace details
{
    ...
    void parallel_for_each_forward_impl(forward_iterator& first, const forward_iterator& last, const function& func, task_group& tg)
    {
        ...
        {
            Concurrency::ppl_extras::details::parallel_for_each_helper<forward_iterator, function> functor(first, last, func);
            tg.run(functor);
        }
        ...
        if (first != last)
        {
            tg.run(
                [&first, &last, &func, &tg]
            {
                Concurrency::ppl_extras::details::parallel_for_each_forward_impl(first, last, func, tg);
            }
            );
        }
    }
...
void _Parallel_reduce_random_executor(_Random_iterator _Begin, _Random_iterator _End, const _Function& _Fun)
{
    ...
    Concurrency::ppl_extras::details::_MallocaArrayHolder<task_handle<_Worker>> _Holder;
...
void _Parallel_reduce_forward_executor(_Forward_iterator _First, _Forward_iterator _Last, const _Function& _Func, task_group& _Task_group)
{
    ...
    Concurrency::ppl_extras::details::_MallocaArrayHolder<task_handle<_Worker_class>> _Holder;
    ...
    size_t _Radix, _Function _Proj_func, const size_t _Chunk_size, size_t _Deep = 0)
{
    ...
    Concurrency::ppl_extras::details::_MallocaArrayHolder<size_t [256]> _Holder;
    ...
    _Function _Proj_func, const size_t _Chunk_size)
{
    ...
    _Integer_type _Max_val = Concurrency::ppl_extras::parallel_reduce(_Begin, _Begin + _Size, _Proj_func(*_Begin), 
